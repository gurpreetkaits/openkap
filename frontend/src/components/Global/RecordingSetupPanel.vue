<template>
  <Teleport to="body">
    <!-- Backdrop -->
    <Transition
      enter-active-class="transition-opacity duration-300"
      leave-active-class="transition-opacity duration-300"
      enter-from-class="opacity-0"
      leave-to-class="opacity-0"
    >
      <div
        v-if="showSetupPanel || isRecording"
        class="fixed inset-0 bg-black/40 backdrop-blur-sm z-40"
        @click="!isRecording && closeSetupPanel()"
      ></div>
    </Transition>

    <Transition
      enter-active-class="transition-all duration-300"
      leave-active-class="transition-all duration-300"
      enter-from-class="opacity-0 translate-x-8"
      leave-to-class="opacity-0 translate-x-8"
    >
      <div
        v-if="showSetupPanel || isRecording"
        ref="cardElement"
        :style="{
          position: 'fixed',
          left: `${cardPosition.x}px`,
          top: `${cardPosition.y}px`,
          zIndex: 50
        }"
        class="w-[340px] max-w-[calc(100vw-2rem)]"
      >
        <div class="card bg-base-100 shadow-2xl">
          <!-- Recording Status View -->
          <div v-if="isRecording" class="card-body p-5">
            <div
              @mousedown="startDrag"
              class="flex items-center justify-between mb-3 cursor-move select-none"
            >
              <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-base-content/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16" />
                </svg>
                <h3 class="text-base font-semibold">Recording</h3>
              </div>
              <span v-if="recordingInterrupted" class="badge badge-warning badge-sm">Interrupted</span>
            </div>

            <!-- Interrupted Warning -->
            <div v-if="recordingInterrupted" class="alert alert-warning mb-4">
              <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <div class="text-xs">
                <strong>Recording Interrupted</strong>
                <p>Your recording was interrupted by a page reload. Please delete this session and start a new recording.</p>
              </div>
            </div>

            <!-- Recording Indicator -->
            <div class="flex items-center gap-3 mb-4 p-3 bg-gradient-to-r from-error/10 to-warning/10 rounded-lg">
              <div class="relative">
                <div v-if="!recordingInterrupted" class="w-2.5 h-2.5 bg-error rounded-full animate-pulse"></div>
                <div v-if="!recordingInterrupted" class="absolute inset-0 w-2.5 h-2.5 bg-error rounded-full animate-ping opacity-75"></div>
                <div v-else class="w-2.5 h-2.5 bg-warning rounded-full"></div>
              </div>
              <div class="flex-1">
                <div class="text-xl font-mono font-bold">{{ formatDuration }}</div>
                <div class="text-xs text-base-content/60">
                  {{ recordingInterrupted ? 'Interrupted' : (isPaused ? 'Paused' : 'Recording...') }}
                </div>
              </div>
              <div class="text-right">
                <div class="text-sm font-medium">{{ formattedUploadedBytes }}</div>
                <div class="text-xs text-base-content/50">uploaded</div>
              </div>
            </div>

            <!-- Mic Waveform Visualizer -->
            <div v-if="microphoneEnabled && !recordingInterrupted" class="mb-4 p-3 bg-base-200 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                <svg class="w-4 h-4 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                </svg>
                <span class="text-xs font-medium">Microphone Active</span>
              </div>
              <div class="flex items-center justify-center gap-0.5 h-8">
                <div
                  v-for="i in 16"
                  :key="i"
                  class="w-1 bg-success rounded-full transition-all duration-75"
                  :style="{
                    height: isPaused ? '4px' : (4 + (micAudioLevel * 24 * Math.sin(i * 0.5 + Date.now() * 0.005))) + 'px',
                    opacity: isPaused ? 0.3 : 0.5 + (micAudioLevel * 0.5)
                  }"
                ></div>
              </div>
            </div>

            <!-- Recording Settings Summary -->
            <div class="space-y-2 text-sm mb-4">
              <div class="flex items-center justify-between p-2.5 bg-base-200 rounded-lg">
                <span class="text-base-content/60">Source</span>
                <span class="font-medium capitalize">{{ selectedSource }}</span>
              </div>
              <div class="flex items-center justify-between p-2.5 bg-base-200 rounded-lg">
                <span class="text-base-content/60">Microphone</span>
                <span :class="microphoneEnabled ? 'text-success' : 'text-base-content/40'">
                  {{ microphoneEnabled ? 'On' : 'Off' }}
                </span>
              </div>
              <div class="flex items-center justify-between p-2.5 bg-base-200 rounded-lg">
                <span class="text-base-content/60">Camera</span>
                <span :class="cameraEnabled ? 'text-success' : 'text-base-content/40'">
                  {{ cameraEnabled ? 'On' : 'Off' }}
                </span>
              </div>
            </div>

            <!-- Recording Controls -->
            <div class="flex items-center gap-2 pt-2 border-t border-base-200">
              <button
                v-if="!isPaused && !recordingInterrupted"
                @click="pauseRecording"
                class="btn btn-ghost flex-1"
                title="Pause Recording"
              >
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" />
                </svg>
                Pause
              </button>

              <button
                v-else-if="!recordingInterrupted"
                @click="resumeRecording"
                class="btn btn-primary flex-1"
                title="Resume Recording"
              >
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M8 5v14l11-7z" />
                </svg>
                Resume
              </button>

              <button
                @click="handleStopRecording"
                :disabled="saving || recordingInterrupted"
                class="btn btn-primary flex-1"
                :class="{ 'btn-disabled': recordingInterrupted }"
                :title="recordingInterrupted ? 'Cannot save interrupted recording' : 'Stop & Save Recording'"
              >
                <span v-if="saving" class="loading loading-spinner loading-sm"></span>
                <svg v-else class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M6 6h12v12H6z" />
                </svg>
                {{ saving ? 'Saving...' : 'Stop' }}
              </button>

              <button
                @click="handleDeleteRecording"
                class="btn btn-error btn-square"
                title="Delete Recording"
              >
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </button>
            </div>
          </div>

          <!-- Setup View -->
          <div v-else class="card-body p-0">
            <!-- Header -->
            <div class="p-5 border-b border-base-200">
              <div
                @mousedown="startDrag"
                class="flex items-center justify-between mb-3 cursor-move select-none"
              >
                <div class="flex items-center gap-2">
                  <svg class="w-4 h-4 text-base-content/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16" />
                  </svg>
                  <svg class="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                  </svg>
                  <h2 class="text-base font-bold">ScreenSense</h2>
                </div>
                <button
                  @click="closeSetupPanel"
                  @mousedown.stop
                  class="btn btn-ghost btn-sm btn-circle"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              </div>

              <!-- Source Selection -->
              <div class="flex items-center gap-2">
                <button
                  @click="selectedSource = 'screen'"
                  :class="[
                    'btn btn-sm flex-1',
                    selectedSource === 'screen' ? 'btn-primary' : 'btn-ghost'
                  ]"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  Screen
                </button>

                <button
                  @click="selectedSource = 'window'"
                  :class="[
                    'btn btn-sm flex-1',
                    selectedSource === 'window' ? 'btn-primary' : 'btn-ghost'
                  ]"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
                  </svg>
                  Window
                </button>

                <button
                  @click="selectedSource = 'tab'"
                  :class="[
                    'btn btn-sm flex-1',
                    selectedSource === 'tab' ? 'btn-primary' : 'btn-ghost'
                  ]"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                  </svg>
                  Tab
                </button>
              </div>
            </div>

            <!-- Recording Settings -->
            <div class="p-5 space-y-3">
              <h3 class="text-sm font-semibold">Recording Settings</h3>

              <!-- Microphone -->
              <div class="form-control">
                <label class="label cursor-pointer">
                  <span class="label-text flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                    </svg>
                    Microphone
                  </span>
                  <input type="checkbox" v-model="microphoneEnabled" class="toggle toggle-primary toggle-sm" />
                </label>
                <select
                  v-if="microphoneEnabled"
                  v-model="selectedMicrophone"
                  class="select select-bordered select-sm w-full mt-2"
                >
                  <option v-for="mic in availableMicrophones" :key="mic.deviceId" :value="mic.deviceId">
                    {{ mic.label || `Microphone ${availableMicrophones.indexOf(mic) + 1}` }}
                  </option>
                </select>
              </div>

              <!-- Camera -->
              <div class="form-control">
                <label class="label cursor-pointer">
                  <span class="label-text flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                    </svg>
                    Camera
                  </span>
                  <input type="checkbox" v-model="cameraEnabled" class="toggle toggle-primary toggle-sm" />
                </label>
                <select
                  v-if="cameraEnabled"
                  v-model="selectedCamera"
                  class="select select-bordered select-sm w-full mt-2"
                >
                  <option v-for="camera in availableCameras" :key="camera.deviceId" :value="camera.deviceId">
                    {{ camera.label || `Camera ${availableCameras.indexOf(camera) + 1}` }}
                  </option>
                </select>
              </div>

              <!-- Advanced Options -->
              <div class="text-xs text-base-content/50 pt-2">
                <p>4K • 60fps • VP9 codec</p>
              </div>
            </div>

            <!-- Actions -->
            <div class="p-5 pt-0 space-y-2">
              <button
                @click="handleStartRecording"
                :disabled="loading"
                class="btn btn-primary btn-block"
              >
                <span v-if="loading" class="loading loading-spinner loading-sm"></span>
                {{ loading ? 'Starting...' : 'Record Now' }}
              </button>
              <button
                @click="closeSetupPanel"
                class="btn btn-ghost btn-block btn-sm"
              >
                Cancel
              </button>
            </div>
          </div>
        </div>
      </div>
    </Transition>

    <!-- Delete Confirmation Modal -->
    <dialog
      class="modal"
      :class="{ 'modal-open': showDeleteConfirm }"
    >
      <div class="modal-box">
        <h3 class="font-bold text-lg">Delete Recording?</h3>
        <p class="py-4 text-base-content/70">
          Are you sure you want to delete this recording? This action cannot be undone.
        </p>
        <div class="modal-action">
          <button @click="showDeleteConfirm = false" class="btn btn-ghost">Cancel</button>
          <button @click="confirmDelete" class="btn btn-error">Delete</button>
        </div>
      </div>
      <form method="dialog" class="modal-backdrop">
        <button @click="showDeleteConfirm = false">close</button>
      </form>
    </dialog>
  </Teleport>
</template>

<script setup>
import { ref, onMounted, onUnmounted } from 'vue';
import { useRouter } from 'vue-router';
import { useRecording } from '../../composables/useRecording';
import toast from '@/services/toastService';

const router = useRouter();

const {
  showSetupPanel,
  isRecording,
  isPaused,
  recordingInterrupted,
  recordingDuration,
  formatDuration,
  formattedUploadedBytes,
  micAudioLevel,
  selectedSource,
  microphoneEnabled,
  cameraEnabled,
  selectedMicrophone,
  selectedCamera,
  availableMicrophones,
  availableCameras,
  closeSetupPanel,
  startRecording,
  pauseRecording,
  resumeRecording,
  stopRecording,
  deleteRecording
} = useRecording();

const loading = ref(false);
const saving = ref(false);
const showDeleteConfirm = ref(false);
const cardElement = ref(null);

// Card position (draggable)
const POSITION_KEY = 'screensense_card_position';
const cardPosition = ref({
  x: window.innerWidth - 340 - 24,
  y: 96
});

// Dragging state
const isDragging = ref(false);
const dragOffset = ref({ x: 0, y: 0 });

const loadPosition = () => {
  try {
    const saved = localStorage.getItem(POSITION_KEY);
    if (saved) {
      const pos = JSON.parse(saved);
      if (pos.x >= 0 && pos.x <= window.innerWidth - 340 &&
          pos.y >= 0 && pos.y <= window.innerHeight - 200) {
        cardPosition.value = pos;
      }
    }
  } catch (error) {
    console.error('Failed to load card position:', error);
  }
};

const savePosition = () => {
  localStorage.setItem(POSITION_KEY, JSON.stringify(cardPosition.value));
};

const startDrag = (e) => {
  isDragging.value = true;
  dragOffset.value = {
    x: e.clientX - cardPosition.value.x,
    y: e.clientY - cardPosition.value.y
  };
  document.addEventListener('mousemove', onDrag);
  document.addEventListener('mouseup', stopDrag);
};

const onDrag = (e) => {
  if (isDragging.value) {
    const newX = e.clientX - dragOffset.value.x;
    const newY = e.clientY - dragOffset.value.y;
    const maxX = window.innerWidth - 340;
    const maxY = window.innerHeight - 200;

    cardPosition.value = {
      x: Math.max(0, Math.min(newX, maxX)),
      y: Math.max(0, Math.min(newY, maxY))
    };
  }
};

const stopDrag = () => {
  if (isDragging.value) {
    isDragging.value = false;
    document.removeEventListener('mousemove', onDrag);
    document.removeEventListener('mouseup', stopDrag);
    savePosition();
  }
};

onMounted(() => {
  loadPosition();
});

onUnmounted(() => {
  document.removeEventListener('mousemove', onDrag);
  document.removeEventListener('mouseup', stopDrag);
});

const handleStartRecording = async () => {
  loading.value = true;
  try {
    await startRecording();
  } catch (error) {
    console.error('Failed to start recording:', error);
    if (error.code === 'VIDEO_LIMIT_REACHED') {
      toast.error(error.message || 'You\'ve reached your video limit. Please upgrade your plan to record more videos.');
    } else if (error.name === 'NotAllowedError') {
      toast.error('Screen sharing was cancelled or denied.');
    } else {
      toast.error('Failed to start recording. Please make sure you grant screen sharing permissions.');
    }
  } finally {
    loading.value = false;
  }
};

const handleStopRecording = async () => {
  saving.value = true;
  try {
    const video = await stopRecording();

    if (!video) {
      toast.error('Recording was interrupted and cannot be saved. Please start a new recording.');
      return;
    }

    router.push(`/video/${video.id}`);
  } catch (error) {
    console.error('Failed to save recording:', error);
    toast.error('Failed to save recording. Please try again.');
  } finally {
    saving.value = false;
  }
};

const handleDeleteRecording = () => {
  showDeleteConfirm.value = true;
};

const confirmDelete = () => {
  deleteRecording();
  showDeleteConfirm.value = false;
};
</script>
