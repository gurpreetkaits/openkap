export const OPENKAP_BASE_URL = process.env.OPENKAP_URL || "https://openkap.com";
/**
...
export function extractShareToken(input) {
    ...
    throw new Error(`Could not parse share token from: "${safe}". ` +
...
export async function apiGet(path) {
    ...
    if (!res.ok) {
        ...
        const message = safeMessages[res.status] ||
 ...
 */
export function buildTranscriptionOutput(v) {
    let output = `# ${v.title || "Untitled Video"}\n\n`;
    output += `**Creator:** ${v.user_name || "Unknown"}\n`;
    output += `**Duration:** ${formatDuration(v.duration)}\n`;
    ...
    if (v.transcription_segments &&
        Array.isArray(v.transcription_segments)) {
        output += `## Timestamped Segments\n\n`;
        for (const seg of v.transcription_segments) {
    ...
    if (comments && comments.length > 0) {
        ...
        for (const c of comments) {
            const at = c.timestamp_seconds
                ? ` at ${formatTimestamp(c.timestamp_seconds)}`
                : "";
 ...
 */
export function buildCommentsOutput(comments) {
    if (comments.length === 0) {
        return "No comments on this video.";
    }
    let output = `## Comments (${comments.length})\n\n`;
    for (const c of comments) {
        const at = c.timestamp_seconds != null
            ? ` [at ${formatTimestamp(c.timestamp_seconds)}]`
            : "";
 ...
 */
export function buildVideoInfo(v, token) {
    return {
        title: v.title,
